...
if(isset($_GET['club_name'])) {
    ...
    $query = "SELECT c.club_name, fc.name AS coordinator_name, fc.email AS coordinator_email, f.dept AS coordinator_department, m.first_name AS member_first_name, m.last_name AS member_last_name, m.email AS member_email, m.phone AS member_phone, m.user_id AS member_user_id
    FROM clubs c
    LEFT JOIN faculty_coordinator fc ON c.club_id = fc.club_id
    LEFT JOIN faculty f ON fc.fid = f.fid
